/**
 * @title Simple Multi-Signature Wallet V2
 * @author Phoenix "CryptoPhoenix" Angell
 * @notice A simple multi-signature smart contract for managing shared ETH and ERC20 token balances.
...
contract SimpleMultiSigV2 is WalletAdmins {
  ...
  // Maps token's ticker hash to contract's total balance
  mapping(bytes32 => uint) public trueBalance;
  // Maps token's ticker hash to contract's pending balance
  mapping(bytes32 => uint) public pendingBalance;
  ...
  function submitTransaction(address payable _recipient, uint _amount, string calldata _tokenTicker) external returns (bool success, uint txID) {
    ...
    require (pendingBalance[tokenTickerBytes] >= _amount, "Insufficient balance");
    ...
    pendingBalance[tokenTickerBytes] -= _amount;
  ...
  function _txnApproval(uint _txID) private returns (bool) {
    ...
    if (numApproved >= adminsRequired) {
      ...
      trueBalance[transaction.ticker] -= transaction.amount;
  ...
  function cancelTransaction(uint _txID) external returns(bool) {
    ...
    pendingBalance[transaction.ticker] += transaction.amount;
  ...
  function depositToken(string calldata _ticker, uint256 _amount) external payable returns(bool) {
    ...
    trueBalance[tickerBytes] += _amount;
    pendingBalance[tickerBytes] += _amount;
  ...
  function modifyTokenAddress(string calldata _ticker, address _tokenAddress) external {
    ...
    tokenAddress[tickerHash(_ticker)] = _tokenAddress;
  }
