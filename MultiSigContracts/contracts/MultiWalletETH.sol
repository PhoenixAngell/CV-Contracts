/*
This is a simple multi-signature smart contract for managing ETH balances.
- Multiple user accounts are managed under this contract.
- Users can deposit ETH, submit transactions, and cancel transactions
- Admins must reach a 2/3 majority to approve a transaction
*/
...
contract MultiWallet is WalletAdmins {
  ...
  function submitTransaction(address payable _recipient, uint _amount) external returns (bool success, uint txID) {
    ...
    // Adjust user's pending balance
     // Note User's current balance is updated when ETH is deposited or transaction is approved
  ...
  function _txnApproved(uint _txID) private returns (bool) {
    ...
    if (numApproved >= adminsRequired) {
      ...
      userBalance[transaction.sender] -= transaction.amount;
  ...
  function cancelTransaction(uint _txID) external {
    ...
    // Update user's pending balance
     // Note User's current balance is updated when ETH is deposited or transaction is approved
    pendingBalance[transaction.sender] += transaction.amount;
